  ...
  return (
    <SafeAreaView style={styles().container}>
      <ScrollView contentContainerStyle={{ flexDirection: "column" }}>
        ...
        {!thereIcons && (
          <ProfileCards name="Elegir Icono" icon="list" onPress={showIcons} />
        )}
        ...
        <View style={styles().iconList}>
          {searchData.map((iconName) => (
            <TouchableOpacity
              style={styles(iconChoosed === iconName).icon}
              onPress={() => chooseIcon(iconName)}
            >
              <FontAwesome
                name={iconName}
                size={20}
                color={iconChoosed === iconName ? "white" : colorText}
              />
            </TouchableOpacity>
          ))}
        </View>
